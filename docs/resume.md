...
- PR #4(`T-032`)로 PostgreSQL이 `docker-compose.db.yml` 별도 스택으로 분리됐고, 14번
...
- PR #5(ADR-005)로 `/health`를 제외한 모든 백엔드 라우트가 `/v1` prefix로 이동했다
  ...
  `RequestValidationError` RFC7807 미적용, `scripts/verify_cutover.py`의 target(14번)
  호출 버저닝 누락도 같은 PR에서 수정했다. `{data, meta}` envelope는 명시적으로 범위
  밖(ADR-005 "후속"). 14번 배포·live 검증 완료(`release_sha=03bd6f3`).
- PR #7(ADR-006)로 공휴일 수집을 `python-kasi-api`(`kasi`)로 옮겼다. hostile review
  P0/P1 없음. 14번 배포·live 검증 완료(`release_sha=986d64e`,
...
- PR #10(`T-029`)으로 ADR-004의 마지막 범위(비행편)를 완료했다. KAC ODCloud
  ...
  `upstream_rate_limit_backoff_seconds` 동안 캐시하도록 고쳤다. 14번 배포·live 검증
